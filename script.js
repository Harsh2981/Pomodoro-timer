...
function changeBackground() {
    ...
    body.style.backgroundImage = `linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('${images[currentImageIndex]}')`;
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        if (timeLeft > 0) {
            ...
            alarmSound.play();
            sessionsCompleted++;
            if (sessionDisplay) sessionDisplay.innerText = sessionsCompleted;
